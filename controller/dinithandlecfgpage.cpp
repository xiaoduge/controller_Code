...
DInitHandleCfgpage::DInitHandleCfgpage(QObject *parent,CBaseWidget *widget ,MainWindow *wndMain) : CSubPage(parent,widget,wndMain)
{
    creatTitle();
...
void DInitHandleCfgpage::creatTitle()
{
    CSubPage::creatTitle();
...
void DInitHandleCfgpage::buildTitles()
{
...
void DInitHandleCfgpage::buildTranslation()
{
...
void DInitHandleCfgpage::switchLanguage()
{
    buildTranslation();
...
void DInitHandleCfgpage::setBackColor()
{
...
void DInitHandleCfgpage::finishSave()
{
    ToastDlg::makeToast(tr("Successfully saved"));
}


void DInitHandleCfgpage::on_CmbIndexChange_trx_type(int index)
{
...
void DInitHandleCfgpage::on_pushButton_FinishBtn()
{
...
void DInitHandleCfgpage::on_pushButton_BackBtn()
{
    m_wndMain->naviInitPage(Ex_Init_Handlercfg, 1);
    m_wndMain->prepareKeyStroke();
}

void DInitHandleCfgpage::initUi()
{
    setBackColor();
...
void DInitHandleCfgpage::on_btn_clicked(int index)
{
    (void)index;
}


void DInitHandleCfgpage::on_pushButton_QueryHandler()
{
    char buf[64];
...
void DInitHandleCfgpage::mouseMoveEvent(QMouseEvent *e)
{
    if (0 == e->x()
        && 0 == e->y())
    {
...
void DInitHandleCfgpage::mouseReleaseEvent(QMouseEvent *e)
{
...
void DInitHandleCfgpage::on_pushButton_CfgHandler()
{
...
void DInitHandleCfgpage::on_pushButton_ResetHandler()
{
    char buf[32];
    IAP_CAN_CMD_STRU *pCmd = (IAP_CAN_CMD_STRU *)buf;
...
void DInitHandleCfgpage::on_pushButton_RmvHandler()
{
    int iLoop;
    QListWidgetItem *pItem;
    HandlerItem *pDevice;
...
void DInitHandleCfgpage::on_pushButton_SaveHandler()
{
    int iLoop;
    int iCount = m_pListWgtHandler->count();
    HandlerItem *pHandler;
...
QListWidgetItem * DInitHandleCfgpage::searchHandler(const QString& info,int iIndex)
{
    int iLoop;
    QListWidgetItem *pItem;
    HandlerItem *pHandler;
...
void DInitHandleCfgpage::on_checkBox_changeState(int state)
{
    (void)state;
...
void DInitHandleCfgpage::update()
{
    on_pushButton_RmvHandler();
    on_pushButton_QueryHandler();
}

void DInitHandleCfgpage::on_pushButton_DeleteHandler()
{
    int iCount = m_pListWgtHandler->count();
    HandlerItem *pHandler;
    QListWidgetItem *pItem;
...
QString DInitHandleCfgpage::getHandler(int iDevIdx,int iInfoIdx)
{
    QListWidgetItem *pItem;
    HandlerItem *pDevice;
...
QListWidgetItem * DInitHandleCfgpage::searchHandlerBySN(const QString& info)
{
   return searchHandler(info,0);

}

QListWidgetItem * DInitHandleCfgpage::searchHandlerByAddress(const QString& info)
{
    return searchHandler(info,1);

}

void DInitHandleCfgpage::addHandler(int iType,const QString& text0,const QString& text1)
{
